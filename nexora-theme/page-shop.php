...
$cat_descs = array(
    'all'         => 'Browse our entire luxury roster across fashion, audio & tech, designer home furnishings, and gourmet groceries.',
    'fashion'     => 'Tailored Italian suiting, pure mulberry silk dresses, and handcrafted leather footwear.',
    'electronics' => 'Studio-grade planar magnetic headphones, audiophile acoustics, and high-end tech.',
...
$page_heading = $is_sale ? 'VIP Flash Vault — Up to 40% Off' : ( isset( $cat_titles[$current_cat] ) ? $cat_titles[$current_cat] : 'The Complete Catalog' );
$page_sub = $is_sale ? 'Time-locked private discounts across our most coveted luxury releases.' : ( isset( $cat_descs[$current_cat] ) ? $cat_descs[$current_cat] : 'Browse our entire luxury roster across fashion, audio & tech, designer home furnishings, and gourmet groceries.' );
?>

<main id="primary" class="site-main">

    <!-- 1. Hero Catalog Banner -->
    <section style="background-color: var(--color-charcoal-dark); color: #fff; padding: 3rem 0; border-bottom: 1px solid rgba(212,168,67,0.3);">
        <div class="container">
            <div style="font-size: 0.8125rem; color: #aaa; margin-bottom: 0.5rem;">
                <a href="<?php echo esc_url( home_url( '/' ) ); ?>" style="color: var(--color-gold); text-decoration: none;"><?php esc_html_e( 'Home', 'nexora-mall' ); ?></a> / <span><?php echo esc_html( $page_heading ); ?></span>
            </div>
            <h1 style="font-size: 2.75rem; color: #ffffff; font-family: var(--font-heading); margin-bottom: 0.5rem;"><?php echo esc_html( $page_heading ); ?></h1>
            <p style="color: #ccc; max-width: 650px; margin: 0; font-size: 1.05rem;">
                <?php echo esc_html( $page_sub ); ?>
            </p>
        </div>
    </section>

    <!-- 2. Shop Main Body with Sidebar & Grid -->
